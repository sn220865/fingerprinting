...
public class FPController extends Controller{
    ...
    public static Result fp() {
        if(request().cookies().get("localhost:9000") == null){
            response().setCookie("tempReturningVis","temp",60*60*12);
            response().setCookie("localhost", UUID.randomUUID().toString(),60*60*24*120,"/","localhost:9000",false,false);
        }
    ...
    public static Result fpNoJs() {
        Http.Cookie cookie = request().cookies().get("localhost:9000");
        String id;
        if(cookie == null){
            id = "Not supported";
        } else {
